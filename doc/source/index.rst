Developer guide for otc-api-sign-sdk-nodejs
============================================

.. toctree::
   :maxdepth: 10
   :hidden:

   Installation <installation/_index>
   Samples <samples/_index>
   Test signing page <testsigningpage/_index>

OpenTelekomCloud Community SDK for signing API requests using NodeJS.

.. warning:: 

   Work in progress. Subject to change without notice.
 


Source Code
-----------

For source code, see :github_repo_master:`otc-api-sign-sdk-nodejs<>` on Github.



.. note:: **Warranty Disclaimer**
    
    THE OPEN SOURCE SOFTWARE IN THIS PRODUCT IS DISTRIBUTED IN THE HOPE THAT IT
    WILL BE USEFUL,BUT WITHOUT ANY WARRANTY; WITHOUT EVEN THE IMPLIED WARRANTY
    OF MERCHANTABILITY OR FITNESS FOR A PARTICULAR PURPOSE.

    SEE THE APPLICABLE LICENSES FOR MORE DETAILS.
